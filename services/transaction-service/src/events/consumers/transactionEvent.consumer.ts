    ...
    eachMessage: async (messagePayload) => {
      ...
      try {
        ...
        switch (eventType) {
          ...
          default:
            logger.warn(`Unhandled event type: ${eventType} for transaction: ${transactionId}`)
        }
      } catch (error) {
...
async function handleAccountCredited(eventData: TransactionEventData): Promise<void> {
  const { transactionId } = eventData
...
async function handleAccountDebitFailed(eventData: TransactionEventData): Promise<void> {
...
async function handleAccountCreditFailed(eventData: TransactionEventData): Promise<void> {
  const { transactionId, error, errorCode } = eventData
...
async function handleAccountDebitCompensated(eventData: TransactionEventData): Promise<void> {
